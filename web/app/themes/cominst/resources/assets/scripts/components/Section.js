...
class Section extends PureComponent {
  ...
  _renderContent() {
    const { ContentContainer, data } = this.props;
  ...
  _switchBackground(child_data) {
    ...
    let image_url = null;
    if (
      child_data.featured_media_metadata &&
      child_data.featured_media_metadata.sizes
    ) {
    ...
    const backgroundImage = image_url
  ...
  componentDidMount() {
    ...
    const inViewportObserver = new IntersectionObserver(
      inViewportObserverCallback,
      inViewportObserverOptions
    );
...
Section.propTypes = {
  ...
  ContentContainer: PropTypes.oneOfType([PropTypes.element, PropTypes.func])
    .isRequired,
