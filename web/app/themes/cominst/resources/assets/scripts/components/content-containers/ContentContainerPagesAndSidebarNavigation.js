...
class ContentContainerPagesAndSidebarNavigation extends Component {
  constructor(props) {
    ...
    this.hasRootPageContent =
      props.data.acf.show_root_content && props.data.content.rendered !== "";
    ...
    this.state = {
      ...
      activeChildTitle:
        this.hasRootPageContent || !data.children.length
          ? data.subtitle
          : data.children[0].title.rendered,
      activeChildSubtitle:
        this.hasRootPageContent || !data.children.length
          ? ""
          : data.children[0].subtitle,
      activeChildContent:
        this.hasRootPageContent || !data.children.length
          ? data.content.rendered
          : data.children[0].introduction,
      activeChildImage:
        this.hasRootPageContent || !data.children.length
          ? data.content.featured_media_html
          : data.children[0].featured_media_html,
      activeChildSecondaryImageObject:
        this.hasRootPageContent || !data.children.length
          ? null
          : data.children[0].acf.secondary_image,
      activeChildNoHeader:
        this.hasRootPageContent || !data.children.length
          ? data.acf.no_header
          : data.children[0].acf.no_header,
      activeChildContentParts:
        this.hasRootPageContent || !data.children.length
          ? data.content_parts
          : data.children[0].content_parts,
  ...
  _setActiveChild(index) {
    ...
    this.setState(
      () => ({
        ...
        activeChildTitle:
          index === null ? data.subtitle : activeChild.title.rendered,
        activeChildSubtitle: index === null ? "" : activeChild.subtitle,
        activeChildContent:
          index === null ? data.content.rendered : activeChild.introduction,
        activeChildImage:
          index === null
            ? data.content.featured_media_html
            : activeChild.featured_media_html,
        activeChildSecondaryImageObject:
          index === null
            ? null
            : activeChild.acf
            ? activeChild.acf.secondary_image
            : null,
        activeChildNoHeader:
          index === null ? data.acf.no_header : activeChild.acf.no_header,
        activeChildContentParts:
          index === null ? data.content_parts : activeChild.content_parts,
        childContentExpanded: false,
      }),
      () => {
        ...
        preloader.preload.apply(this, images).then(() => {
          _component.setState(() => {
            return {
              activeChildContentStyles: {
                height:
                  _component._childContentContainerRef.children[0].clientHeight,
              },
            };
          });
        });
  ...
  _childTitleClickHandler(index) {
    ...
    this.props.dataCallback(
      index !== null ? this.props.data.children[index] : this.props.data
    );
  }
  ...
  componentDidMount() {
    ...
    const children_with_index = this.props.data.children.map(
      (child, index) => ({ index, slug: child.slug })
    );
    const children = children_with_index.filter(child => child.slug === slug);
    const child_index = children.length
      ? children[0].index
      : this.hasRootPageContent || !children_with_index.length
      ? null
      : 0;
  ...
  _getSubSectionSlug() {
    const match = this.props.location.pathname.match(
      this.props.parent.props.path
    );
    if (match) {
      const path_parts = this.props.location.pathname
        .replace(/^\/(.*)?\/$/, "$1")
        .split("/");
  ...
  _renderNav() {
    ...
    const _parentMenuItemArray = appData.primary_navigation.filter(
      item => data.id === item.object_id
    );
    ...
    if (_parentMenuItemArray.length) {
      ...
      _children = data.children.map(child => {
        const _childMenuItem = _parentMenuItem.children
          ...
          <li key={child.id} className="item">
            <Link
              className={classNames({
                active: index === this.state.activeChildIndex,
              })}
            ...
            {this.hasRootPageContent || data.children.length ? (
              <div>
                <div
                  ...
                  style={this.state.activeChildContentStyles}
                  ref={element => (this._childContentContainerRef = element)}
                >
                  <div>
                    <Page
                      ...
                      secondaryImageObject={
                        this.state.activeChildSecondaryImageObject
                      }
                      noHeader={this.state.activeChildNoHeader}
                ...
                <div
                  className="child-image image"
                  dangerouslySetInnerHTML={{
                    __html: this.state.activeChildImage,
                  }}
                />
              </div>
            ) : (
