...
const ContentContainerFooter = function({ data }) {
  try {
    if (!data.mailchimp_subscribe_url) {
      throw new Error(
        "You must define a mailchimp_subscribe_url for your site"
      );
    }
  } catch (error) {
    ...
    <div className="content-container content-container-footer">
        ...
        <div className="col-md-4 col-right">
            ...
            <ul className="social-networks">
              {data.social_networks.map(network => (
                <li className="item" key={network.name}>
                  ...
                  >
                    <i
                      className={`icon fa fa-${network.name.toLowerCase()}-square`}
                      aria-hidden="true"
                    />
                  </a>
                </li>
          ...
          <div className="block block-newsletters-container">
            ...
            <MailchimpSubscribeForm
              ...
              messages={{
                inputPlaceholder:
                  appData.i18n.newsletterSubscribeForm.inputPlaceholder,
          ...
          <div className="block legal-infos-container">
            ...
            <ul className="legal-infos-links">
              {data.secondary_navigation.items.map(item => (
      ...
      <div className="row signature">
        <div className="col-sm-12">
          <span className="site-name">© {data.site_name}</span>{" "}
          <span className="separator">-</span>{" "}
          <span className="site-description">{data.site_description}</span>
        </div>
      </div>
    </div>
  );
